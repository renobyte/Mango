...
public class SyncService extends Service
{
    ...
    private void checkForNewChapters()
    {
        Runnable r = new Runnable()
        {
            ...
            public void run()
            {
                ...
                {
                    Favorite[] n = null;
                    db = new MangoSqlite(SyncService.this);
                    db.open();
                    ...
                    for (int i = 0; i < n.length; i++)
                    {
                        try
                        {
                            ...
                            String response = MangoHttp.downloadData(
                                    "http://%SERVER_URL%/getchapterlist.aspx?pin=" + Mango.getPin() + "&url=" + n[i].mangaId + "&site=" + n[i].siteId + "&latest=true", SyncService.this);
                            ...
                            // Mango.Log(n[i].mangaId + ": Current = " + n[i].lastChapterId + ", Latest = " + c.id +
                            ...
                            if (!c.id.equals(n[i].lastChapterId)) // new chapter
                            {
                                ...
                                if (Mango.getSharedPreferences().getBoolean("notifierAutoDownload", false))
                                {
                                    ...
                                    reader.parse(new InputSource(new StringReader(MangoHttp.downloadData(
                                            "http://%SERVER_URL%/getchapterlist.aspx?pin=" + Mango.getPin() + "&url=" + n[i].mangaId + "&site=" + n[i].siteId, SyncService.this))));
    ...
    private String fixDate(String date)
    {
        ...
        if (date.equalsIgnoreCase("today"))
            newdate = DateFormat.getDateFormat(SyncService.this).format(new Date(System.currentTimeMillis()));
        else if (date.equalsIgnoreCase("yesterday"))
            newdate = DateFormat.getDateFormat(SyncService.this).format(new Date(System.currentTimeMillis() - 86400000));
