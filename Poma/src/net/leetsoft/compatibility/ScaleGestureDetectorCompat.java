package net.leetsoft.compatibility;
...
/**
 * Detects transformation gestures involving more than one pointer ("multitouch") using the supplied {@link MotionEvent}
 * s. The {@link OnScaleGestureListener} callback will notify users when a particular gesture event has occurred. This
 * class should only be used with {@link MotionEvent}s reported via touch.
 * <p/>
 * To use this class:
 * <ul>
 * <li>Create an instance of the {@code ScaleGestureDetectorCompat} for your {@link View}
...
public class ScaleGestureDetectorCompat
{
     ...
     * <ul>
     * <li>One {@link OnScaleGestureListener#onScaleBegin(ScaleGestureDetectorCompat)}
     * <li>Zero or more {@link OnScaleGestureListener#onScale(ScaleGestureDetectorCompat)}
     * <li>One {@link OnScaleGestureListener#onScaleEnd(ScaleGestureDetectorCompat)}
     * </ul>
     */
    public interface OnScaleGestureListener
    {
         ...
         * <p/>
         * Once a scale has ended, {@link ScaleGestureDetectorCompat#getFocusX()} and
         * {@link ScaleGestureDetectorCompat#getFocusY()} will return the location of the pointer remaining on the
         * screen.
         *
         * @param detector The detector reporting the event - use this to retrieve extended info about event state.
         */
     ...
     * {@link OnScaleGestureListener#onScale(ScaleGestureDetectorCompat)} returns {@code false} so that a subclass can
     * retrieve the accumulated scale factor in an overridden onScaleEnd.
     * {@link OnScaleGestureListener#onScaleBegin(ScaleGestureDetectorCompat)} returns {@code true}.
     */
        ...
        if (!mGestureInProgress)
        {
            switch (action & MotionEvent.ACTION_MASK)
            {
                case MotionEvent.ACTION_POINTER_DOWN:
                {
                    ...
                    if (x1 == -1 || y1 == -1)
                    {
                        Mango.log("ScaledGestureDetectorCompat is discarding an invalid MotionEvent.");
                        return false;
                    }
                ...
                case MotionEvent.ACTION_POINTER_UP:
                    if (mSloppyGesture)
                    {
                        ...
                        int id = (((action & ACTION_POINTER_INDEX_MASK) >> ACTION_POINTER_INDEX_SHIFT) == 0) ? 1 : 0;
